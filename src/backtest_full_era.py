"""
OCTAGON AI - FULL ERA BACKTEST (2010-2024)
==========================================
Strict n+1 Protocol: At Time T, only data from Time < T is available.

Protocol:
1. Start with empty stats tracker (no pre-loading)
2. For each fight chronologically:
   - EXTRACT: Get fighter's pre-fight stats (empty for debutants)
   - PREDICT: Make prediction using lagged stats
   - BET: Log result if edge exists
   - UPDATE: Update tracker with fight outcome
3. Era-aware: No normalization that uses future data
"""
...
def calculate_kelly_stake(model_prob, market_prob, decimal_odds, bankroll, fraction=0.25):
    ...
    # OPTIMIZED: 12% minimum edge, 17% max (higher edges are unreliable)
    if edge <= 0.12 or edge > 0.17:
        return 0.0
    ...
    stake = min(stake, bankroll * 0.03)  # 3% max (conservative for long-term)
...
def run_backtest():
    print("="*60)
    print("OCTAGON AI - FULL ERA BACKTEST (2010-2024)")
    print("="*60)
    ...
    # Load the new comprehensive odds file
    ...
    # === N+1 PROTOCOL: Empty stats tracker at start ===
    print("Starting n+1 strict backtest (empty tracker)...")
    stats_tracker = {}
    
    # Match fights with odds and results
    print("Matching fights with odds...")
    fight_odds_pairs = []
    seen_fights = set()  # Prevent duplicate bets on same fight
    ...
    for _, odds_row in odds_df.iterrows():
        ...
        if len(fight_match) == 0:
            continue
        
        fight_row = fight_match.iloc[0]
        flipped = (fight_row['Fighter_Id_1'] == fid2)
        
        # Create unique fight key to prevent duplicates
    ...
    print(f"Found {len(fight_odds_pairs)} fights with odds from 2010-2024")
    
    # === MAIN BACKTEST LOOP ===
    initial_bankroll = 1000.0
    bankroll = initial_bankroll
    high_water = initial_bankroll
    max_drawdown = 0.0
    ...
    # Track by year for era analysis
    yearly_stats = {}
    ...
    for idx, item in enumerate(fight_odds_pairs):
        ...
        if year not in yearly_stats:
            yearly_stats[year] = {'bets': 0, 'wins': 0, 'pnl': 0.0}
        
        # Correct odds order based on fight order
        if flipped:
            real_fid1 = fid2
            real_fid2 = fid1
        ...
        # --- EXTRACT: Get PRE-FIGHT stats (n+1 rule) ---
        ...
        # Skip if neither fighter has fought before (pure coin flip)
        if st1['total_fights'] < 2 and st2['total_fights'] < 2:
            # Still update stats after
            try:
        ...
        if stake > 0:
            ...
            if bankroll > high_water:
                high_water = bankroll
            drawdown = (high_water - bankroll) / high_water
        ...
        # --- UPDATE: Update stats AFTER prediction ---
        try:
            time_sec = parse_time(fight_row['Fight_Time'], fight_row['Round'])
            str1 = float(fight_row.get('STR_1', 0) or 0)
            str2 = float(fight_row.get('STR_2', 0) or 0)
        ...
        if idx % 1000 == 0 and idx > 0:
            print(f"  Processed {idx}/{len(fight_odds_pairs)} fights... Bankroll: ${bankroll:.2f}")
    
    # Report
    print("\n" + "="*60)
    print("FULL ERA BACKTEST RESULTS (2010-2024)")
    print("="*60)
    ...
    for year in sorted(yearly_stats.keys()):
        ...
        if ys['bets'] > 0:
            wr = ys['wins'] / ys['bets'] * 100
            print(f"{year}: {ys['bets']} bets, {ys['wins']}W ({wr:.0f}%), P/L: ${ys['pnl']:.2f}")
    
    if results:
        results_df = pd.DataFrame(results)
        results_df.to_csv(os.path.join(data_dir, 'backtest_full_era_2010_2024.csv'), index=False)
        print(f"\nDetailed results saved to backtest_full_era_2010_2024.csv")
